document.addEventListener("DOMContentLoaded", async () => {
    ...
    const loadProducts = async (category = "") => {
        try {
            const response = await fetch(`http://localhost:22951/api/Productos/GetProductos?categoria=${encodeURIComponent(category)}`);
    ...
    const loadCategories = async () => {
        try {
            const response = await fetch("http://localhost:22951/api/Productos/GetCategorias");
