...
function fetchMovieData(movieTitle) {
    const apiUrl = 'http://3.210.64.89:80/api/Movie/GetPeliculas';
...
function fetchProductData(productIds) {
    const apiUrl = `http://3.210.64.89:80/api/Productos/GetProductos?ids=${productIds.join(',')}`;
...
async function createOrder() {
    ...
    try {
        const response = await fetch('http://3.210.64.89:80/api/Pedido/CreatePedido', {
            method: 'POST',
