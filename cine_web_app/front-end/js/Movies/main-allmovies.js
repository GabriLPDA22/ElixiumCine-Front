document.addEventListener('DOMContentLoaded', () => {
    const API_BASE_URL = "http://localhost:22951/api/Movie"; // Cambia el puerto si es necesario
...
async function loadMovies() {
    try {
        ...
        const savedFilter = localStorage.getItem('selectedFilter') || 'all';
        let url = 'http://localhost:22951/api/Movie/GetPeliculas'; // Default: todas las películas

        // Ajustar la URL según el filtro seleccionado
        switch (savedFilter) {
            case 'inTheater':
                url = 'http://localhost:22951/api/Movie/GetPeliculasEnCartelera';
                break;
            case 'comingSoon':
                url = 'http://localhost:22951/api/Movie/GetPeliculasProximas';
                break;
            case 'preSale':
                url = 'http://localhost:22951/api/Movie/GetPeliculasEnVentaAnticipada';
                break;
        }
