...
async function loadGuestPurchaseInfo() {
    ...
    try {
        const response = await fetch(`http://localhost:22951/api/Cine/GetSeatSelectionInfo?cineName=${encodeURIComponent(cinemaName)}&movieTitle=${encodeURIComponent(movieTitle)}&sessionDate=${encodeURIComponent(date)}&sessionTime=${encodeURIComponent(time)}`);
