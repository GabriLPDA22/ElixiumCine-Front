...
async function loadGuestPurchaseInfo() {
    ...
    try {
        const response = await fetch(`http://3.210.64.89:3000/api/Cine/GetSeatSelectionInfo?cineName=${encodeURIComponent(cinemaName)}&movieTitle=${encodeURIComponent(movieTitle)}&sessionDate=${encodeURIComponent(date)}&sessionTime=${encodeURIComponent(time)}`);
