...
async function loadGuestPurchaseInfo() {
    ...
    try {
        const response = await fetch(`http://3.210.64.89:80/api/Cine/GetSeatSelectionInfo?cineName=${encodeURIComponent(cinemaName)}&movieTitle=${encodeURIComponent(movieTitle)}&sessionDate=${encodeURIComponent(date)}&sessionTime=${encodeURIComponent(time)}`);
